...
def build_and_train_v2(
    ...
    img_height_orig = int(img.height)
    img_width_orig = int(img.width)
    img_height_augmented = int(img_height_orig * resize_ratio)
    img_width_augmented = int(img_width_orig * resize_ratio)
    ...
    train_ds = tf.keras.utils.image_dataset_from_directory(
        ...
        subset="training",
        seed=123,
        image_size=(img_height_orig, img_width_orig),
    ...
    model = Sequential([
        layers.Input(shape=(img_height_orig, img_width_orig, 3)),
        layers.Cropping2D(cropping=((crop[1], crop[3]), (crop[0], crop[2]))),
        layers.Resizing(img_height_augmented, img_width_augmented, interpolation='bilinear'),
    ...
    features = np.concatenate([val_data[n][0] for n in range(0, len(val_data))])
    targets = np.concatenate([val_data[n][1] for n in range(0, len(val_data))])
    print(targets)
    ...
    rendering = {
        'image_resize': [img_height_orig, img_width_orig],
        'classes': class_names
    }
